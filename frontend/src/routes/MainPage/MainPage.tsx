...
const TRACKING_ID = 'G-GSGHDWWRD3';

ReactGA.initialize(TRACKING_ID);
export default function App() {
  const [lang, setLang] = useState<LangsType>('en');
  const [step, setStep] = useState<number>(0);
  const gaEventTracker = useAnalyticsEventTracker('Test Expert Option Bot');
  ...
  return (
    <div className='relative flex flex-col items-center justify-between w-full h-full text-white bg-[#212A40]'>
      <div className='w-full flex flex-col items-center justify-center gap-5'>
        ...
        {step === 1 && (
          <div className='mt-2 relative w-full h-full flex flex-col items-center justify-center max-w-[320px] text-center gap-2'>
            <h2 className='font-bold text-center text-2xl z-2 w-[185px]'>
              Investing is Even Better Now
            </h2>
            <p className='z-20 text-center text-[15px] text-white w-[310px]'>
              Providing you the opportunity to invest in more than 100 assets
        ...
        {step === 2 && (
          <div className='mt-2 relative w-full h-full flex flex-col items-center justify-center max-w-[300px] text-center gap-2'>
            <h2 className='font-bold text-2xl z-[30]'>
              {Langs['en'].user_testimonials}
            </h2>
            <p className='z-20 text-center text-[15px] text-white'>
              {Langs['en'].join_quotex}
            </p>
          </div>
        )}
      </div>
      ...
      {step === 0 && (
        <div className='mt-[-10vh] flex items-center justify-center h-full w-full'>
          <img src='/logo.svg' alt='logotype' width={195} height={35} />
        </div>
      )}
      ...
      {step === 2 && (
        <div className='relative w-full h-full flex flex-col items-center justify-center text-center gap-10'>
          ...
          />
          <img
            src={`/images/png/en/2-another.png`}
            alt='Second part'
      ...
      {step > 0 && (
        <div
          ...
          style={{
            background:
              'linear-gradient(0deg, #212A40 0%, #212A40 75.55%, rgba(16, 44, 82, 0) 96.71%)',
          }}
        >
          <StepBar active={step} />
